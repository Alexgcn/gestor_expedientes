...
const renderRecords = async (url,page,size) => {
    ...
    const html = expedientes.map((expediente,index) => {
        ...
        <td>${expediente.secretario}</td>
            ...
            <td>
                <a href="/expedientes/buscar/${expediente.id}" class="btn btn-outline btn-sm btn-show "><!--<i class="bi bi-eye-fill"></i>-->Observar</a>
                <a href="/expedientes/editar/${expediente.id}" class="btn btn-outline-success btn-sm mt-1 btnEdit"><!--<i class="bi bi-pencil-fill"></i>-->Editar</a>
            </td>
        </tr>
        `;
    }).join("");
